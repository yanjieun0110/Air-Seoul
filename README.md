...
## /* 공항 하이라이트 + 출발지 제외 js */
→ 출발지와 도착지를 선택할 때 `.ch_chk` 클래스를 적용하여 선택 상태를 표시했습니다. 또한 현재 선택 타입(출발지 / 도착지)을 구분하여 선택된 공항 정보를 관리하고, 출발지로 선택된 공항은 도착지 목록에서 보이지 않도록 처리하여 동일 공항을 중복 선택할 수 없도록 구현했습니다.
